...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print("qkvqkvqkvqkqvkqvkvqqkqkqkqvvv") # XYZ TODO 
        # print('qqqqqqqq', q[0] , '\n') # XYZ TODO
        # print('kkkkkkkk', k[0] , '\n') # XYZ TODO
        # print('vvvvvvvv', v[0] , '\n') # XYZ TODO
        ...
        # print("atttttttttttttttttttttttttttttn\n")
        # print(attn)
...
class CosformerAttention(nn.Module):
    ...
    def forward(
        ...
        x = x.type(torch.float32) # XYZ TODO
        # print("#######################", x.dtype)
        if query == None: # XYZ TODO
            query = x # XYZ TODO
        # print("#######################", query.dtype)
        query = query.type(torch.float32) # XYZ TODO
        if key == None:
            key = query
        key = key.type(torch.float32) # XYZ TODO
        if value == None:
            value = query
        value = value.type(torch.float32) # XYZ TODO
        ...
        # print("#######################", query.dtype)
        # bsz, tgt_len, embed_dim = query.size() # XYZ TODO
        src_len = key.size(0)
        # src_len = key.size(1) # XYZ TODO
        ...
        # print("coscoscoscsocsoocsocosocso", q ,k ,v) # XYZ TODO
        ...
        if self.causal:
            ...
            q_ = q_.type(torch.float32) # XYZ TODO
            z_ = 1 / torch.clamp_min(torch.einsum('nld,nd->nl', q_, torch.sum(k_, axis=1)), eps)
            # print("#######################", q_.dtype, torch.sum(k_, axis=1).dtype) # XYZ TODO
            # (N * h, L, 2 * d) (N * h, d, 2 * d) (N * h, L) -> (N * h, L, d)
            kv_ = kv_.type(torch.float32) # XYZ TODO
            z_ = z_.type(torch.float32) # XYZ TODO
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, i_layer, dim, num_heads, window_size=7, shift_size=0, # XYZ TODO
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.i_layer = i_layer # XYZ TODO
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.cosattn = CosformerAttention(embed_dim=dim, num_heads=num_heads, causal=False) # XYZ TODO
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.i_layer == 2 or 3: # XYZ TODO
            x = x.view(B, H * W, C) # XYZ TODO
            x = x.transpose(0,1) # XYZ TODO
            cosattn = self.cosattn(x = x, query = x) # XYZ TODO
            cosattn = cosattn.transpose(0,1) # XYZ TODO
            x = cosattn # XYZ TODO
...
class CosSwinTransformer(nn.Module):
    """ XYZ COS Swin Transformer backbone.
    ...
    def train(self, mode=True):
        ...
        super(CosSwinTransformer, self).train(mode) # XYZ TODO
        self._freeze_stages()
