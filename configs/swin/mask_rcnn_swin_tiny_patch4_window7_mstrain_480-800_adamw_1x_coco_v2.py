...
model = dict(
    ...
    # neck=dict(in_channels=[192, 384, 768, 1536])) # XYZ TODO 2022.12.12
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True ,with_lms=True),
    dict(
    ...
    # dict(type='RandomFlip', flip_ratio=1), # XYZ TODO 2022.11.11
    dict(type='Pad', size_divisor=32), # 15.Pad an image to ensure each edge to be multiple to some number
    dict(type='DefaultFormatBundle'),
    dict(type='Collect', keys=['img', 'gt_bboxes','gt_lms', 'gt_labels']), # XYZ TODO
]
